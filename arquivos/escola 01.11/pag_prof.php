...
<head>
    ...
    <link rel="stylesheet" href="pag_prof style.css">
...
if ($_SESSION['user_role'] === 'admin') {
    ...
    $query = "SELECT * FROM usuarios WHERE email = ?"; //espaço reservado para o email.
    
    $stmt = $conn->prepare($query); //preparar declaração.
    
    $stmt->bind_param("s", $email);// linkar email com o espaço reservado ao usuário (professor). "s" significa que o tipo de dado é uma string... podem haver 'i' - inteiros; 'd' - double/decimal ou 'b' - binário.
    
    $stmt->execute(); //executar declaração.
...
<?php
if (!empty($row)) {
        echo '<p> olá professor ' . $row["nome"] . '!<br></p>';
    }
} else {
